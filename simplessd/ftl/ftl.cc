...
FTL::FTL(ConfigReader &c, DRAM::AbstractDRAM *d, CPU::CPU *pCPU, ESPRESSO::ESPRESSO *pESPRESSO) : conf(c), pDRAM(d), pCPU(pCPU), pESPRESSO(pESPRESSO) {
  PAL::Parameter *palparam;

  pESPRESSO->setFTL(this);

  pPAL = new PAL::PAL(conf, pESPRESSO);
  ...
  switch (conf.readInt(CONFIG_FTL, FTL_MAPPING_MODE)) {
    case PAGE_MAPPING:
      pFTL = new PageMapping(conf, param, pPAL, pESPRESSO, pDRAM, pESPRESSO->pRemoteDRAM, pCPU);
      break;
  }
...
void FTL::updateCommit() {
  ...
  do {
    ...
    if (find) {
      ...
      uint64_t debug = iter->first.reqID;
      debugprint(LOG_FTL, "LPN %" PRIu64, debug);
      CPU::FUNCTION func = CPU::READ;
      if (newreq.reqType == WRITE) func = CPU::WRITE;
      if (newreq.reqType == TRIM) func = CPU::TRIM;
      pCPU->applyLatency(CPU::FTL, func, newreq.finishAt);
...
void FTL::format(LPNRange &range, uint64_t &tick) {
  ...
  pCPU->applyLatency(CPU::FTL, CPU::FORMAT, tick);
}
...
void FTL::submitRemotePALReq(PAL::Request &req, uint64_t &tick) {
  pFTL->submitRemotePALReq(req, tick);
}

void FTL::triggerRemoteUpdateSubmit() {
  pFTL->triggerRemoteUpdateSubmit();
}

void FTL::commitRemotePALReq(PAL::Request &req) {
  pFTL->commitRemotePALReq(req);
}

void FTL::commitRemoteFTLReq(PAL::Request req) {
  pFTL->commitRemoteFTLReq(req);
}

bool FTL::conductRemoteFTLMapFind(uint64_t idx, uint64_t &tick, void *iter, bool feedMRC) {
  return pFTL->conductRemoteFTLMapFind(idx, tick, iter, feedMRC);
}

bool FTL::conductRemoteFTLBlocksFind(uint32_t idx, void *iter) {
  return pFTL->conductRemoteFTLBlocksFind(idx, iter);
}

void FTL::conductRemoteFTLMapModify(uint64_t idx, uint64_t &tick) {
  pFTL->conductRemoteFTLMapModify(idx, tick);
}

void FTL::conductRemoteFTLMapEmplaceEmpty(uint64_t idx) {
  pFTL->conductRemoteFTLMapEmplaceEmpty(idx);
}

bool FTL::conductRemoteFTLGetBlockToWrite(uint32_t nPage, void *iter) {
  return pFTL->conductRemoteFTLGetBlockToWrite(nPage, iter);
}
