...
Subsystem::Subsystem(Controller *ctrl, ConfigData &cfg, ESPRESSO::ESPRESSO *pESPRESSO, CPU::CPU *pCPU)
...
void Subsystem::init() {
  pHIL = new HIL(conf, pCPU, pESPRESSO);
    ...
    if (req.entry.namespaceID < NSID_ALL) {
      for (auto &iter : lNamespaces) {
        if (iter->getNSID() == req.entry.namespaceID) {
          ...
          pCPU->execute(CPU::NVME__NAMESPACE, CPU::SUBMIT_COMMAND, doSubmit,
                  pContext);
      ...
      if (lNamespaces.size() > 0) {
        ...
        pCPU->execute(CPU::NVME__NAMESPACE, CPU::SUBMIT_COMMAND, doSubmit, pContext);
...
void Subsystem::read(Namespace *ns, uint64_t slba, uint64_t nlblk,
  ...
  IOContext *ctx = (IOContext *)context;
  
  req->reqID = ctx->resp.entry.dword3.commandID;

  if (ctx->isRemoteReq) {
    req->isRemoteReq = ctx->isRemoteReq;
    req->isSimpleSolution = ctx->isSimpleSolution;
    req->debtorSSDID = ctx->debtorSSDID;
    req->creditorSSDID = ctx->creditorSSDID;
  }
  ...
  pCPU->execute(CPU::NVME__SUBSYSTEM, CPU::CONVERT_UNIT, doRead, req);
}
...
void Subsystem::write(Namespace *ns, uint64_t slba, uint64_t nlblk,
  ...
  pCPU->execute(CPU::NVME__SUBSYSTEM, CPU::CONVERT_UNIT, doWrite, req);
}

void Subsystem::flush(Namespace *ns, DMAFunction &func, void *context) {
  Request req(func, context);
  assert(!((IOContext *) context)->resp.isRemoteReq);
...
void Subsystem::trim(Namespace *ns, uint64_t slba, uint64_t nlblk,
  ...
  pCPU->execute(CPU::NVME__SUBSYSTEM, CPU::CONVERT_UNIT, doTrim, req);
}
...
bool Subsystem::createCQueue(SQEntryWrapper &req, RequestFunction &func) {
  ...
  bool isCreditorQueue = req.entry.dword11 & 0x04;
  ...
  if (!err) {
    ...
    int ret = pParent->createCQueue(cqid, entrySize, iv, isCreditorQueue, ien, pc,
                                    req.entry.data1, doQueue, pContext);
...
bool Subsystem::namespaceAttachment(SQEntryWrapper &req,
  ...
  if (req.useSGL) {
    pContext->dma =
        new SGL(cfgdata, doRead, pContext, req.entry.data1, req.entry.data2, pCPU);
  }
  else {
...
bool Subsystem::formatNVM(SQEntryWrapper &req, RequestFunction &func) {
  ...
  if (err) {
    ...
    if (info) {
      ...
      pCPU->execute(CPU::NVME__SUBSYSTEM, CPU::FORMAT_NVM, job, req);
    }
    else {
      resp.makeStatus(false, false, TYPE_GENERIC_COMMAND_STATUS,
                      STATUS_ABORT_INVALID_NAMESPACE);
