...
Namespace::Namespace(Subsystem *p, ConfigData &c, ESPRESSO::ESPRESSO *pESPRESSO, CPU::CPU *pCPU)
    : pParent(p),
      pDisk(nullptr),
      pESPRESSO(pESPRESSO),
      pCPU(pCPU),
...
void Namespace::flush(SQEntryWrapper &req, RequestFunction &func) {
  ...
  if (!err) {
    ...
    pCPU->execute(CPU::NVME__NAMESPACE, CPU::FLUSH, begin, pContext);
  }
}
...
void Namespace::write(SQEntryWrapper &req, RequestFunction &func) {
  ...
  if (!err) {
    ...
    CPUContext *pCPUContext =
        new CPUContext(doRead, pContext, CPU::NVME__NAMESPACE, CPU::WRITE, pCPU);
    ...
    if (req.useSGL) {
      ...
      pContext->dma =
          new PRPList(cfgdata, cpuHandler, pCPUContext, req.entry.data1,
                      req.entry.data2, (uint64_t)nlb * info.lbaSize, pCPU);
    }
  }
  else {
    func(resp);
...
void Namespace::read(SQEntryWrapper &req, RequestFunction &func) {
  ...
  if (!err) {
    DMAFunction doRead = [this](uint64_t tick, void *context) {
      ...
      pContext->dma->write(0, pContext->nlb * info.lbaSize, pContext->buffer, dmaDone, context, true);
    };
    ...
    CPUContext *pCPUContext =
        new CPUContext(doRead, pContext, CPU::NVME__NAMESPACE, CPU::READ, pCPU);
    ...
    if (req.useSGL) {
      ...
      pContext->dma =
          new PRPList(cfgdata, cpuHandler, pCPUContext, req.entry.data1,
                      req.entry.data2, pContext->nlb * info.lbaSize, pCPU);
    }
  }
  else {
    func(resp);
...
void Namespace::datasetManagement(SQEntryWrapper &req, RequestFunction &func) {
  ...
  if (!err) {
    ...
    CPUContext *pCPUContext = new CPUContext(doTrim, pContext, CPU::NVME__NAMESPACE,
