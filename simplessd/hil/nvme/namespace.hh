...
class IOContext : public RequestContext {
 public:
  ...
  bool isRemoteReq = false;
  bool isSimpleSolution = false;
  uint8_t debtorSSDID = 0xFF;
  uint8_t creditorSSDID = 0xFF;

  IOContext(RequestFunction &f, CQEntryWrapper &r) : RequestContext(f, r), 
            isRemoteReq(r.isRemoteReq), isSimpleSolution(r.isSimpleSolution), debtorSSDID(r.debtorSSDID), creditorSSDID(r.creditorSSDID) {}
};
...
class Namespace {
 ...
 public:
  Namespace(Subsystem *, ConfigData &, ESPRESSO::ESPRESSO *, CPU::CPU *);
  ~Namespace();
