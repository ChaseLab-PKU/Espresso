...
Controller::Controller(Interface *intrface, ConfigReader &c, CPU::CPU *pCPU, uint8_t ssdID, bool simpleSolution, std::string od)
    : pParent(intrface),
      pCPU(pCPU),
      ssdID(ssdID),
  ...
  // Init ESPRESSO
  pESPRESSO = new ESPRESSO::ESPRESSO(conf, pCPU, this, ssdID, output_dir);

  cfgdata.pConfigReader = &c;
  cfgdata.pInterface = interconnect;
  cfgdata.pESPRESSO = pESPRESSO;
  ...
  if (vid == OCSSD_VENDOR) {
    ...
    switch (ssvid) {
      case OCSSD_SSVID_1_2:
        pSubsystem = new OpenChannelSSD12(this, cfgdata, pESPRESSO, pCPU);

        break;
      case OCSSD_SSVID_2_0:
        pSubsystem = new OpenChannelSSD20(this, cfgdata, pESPRESSO, pCPU);
    ...
    pSubsystem = new Subsystem(this, cfgdata, pESPRESSO, pCPU);
  }
...
void Controller::writeRegister(uint64_t offset, uint64_t size, uint8_t *buffer,
        ...
        if (ppCQueue[0]) {
          ppCQueue[0]->setBase(
              new PRPList(cfgdata, empty, nullptr,
                          ...
                          ppCQueue[0]->getSize() * cqstride, true, pCPU),
              cqstride);
        }
        if (ppSQueue[0]) {
          ppSQueue[0]->setBase(
              new PRPList(cfgdata, empty, nullptr,
                          ...
                          ppSQueue[0]->getSize() * sqstride, true, pCPU),
              sqstride);
        }
  ...
  if (adminQueueInited == 4) {
    ...
    ppCQueue[0] = new CQueue(0, false, true, 0, entrySize);
...
int Controller::createCQueue(uint16_t cqid, uint16_t size, uint16_t iv, bool isCreditorQueue,
                             bool ien, bool pc, uint64_t prp1,
                             DMAFunction &func, void *context) {
  int ret = 1;  // Invalid Queue ID
  CPUContext *pContext =
      new CPUContext(func, context, CPU::NVME__CONTROLLER, CPU::CREATE_CQ, pCPU);

  if (ppCQueue[cqid] == NULL) {
    ppCQueue[cqid] = new CQueue(iv, isCreditorQueue, ien, cqid, size);
    ppCQueue[cqid]->setBase(
        new PRPList(cfgdata, cpuHandler, pContext, prp1, size * cqstride, pc, pCPU),
        cqstride);

    ret = 0;

    debugprint(LOG_HIL_NVME,
               "CQ %-5d| CREATE | Entry size %d | IV %04X | ISCREDITOR %s | IEN %s | PC %s",
               cqid, size, iv, BOOLEAN_STRING(isCreditorQueue), BOOLEAN_STRING(ien), BOOLEAN_STRING(pc));
...
int Controller::createSQueue(uint16_t sqid, uint16_t cqid, uint16_t size,
  ...
  int ret = 1;  // Invalid Queue ID
  CPUContext *pContext =
      new CPUContext(func, context, CPU::NVME__CONTROLLER, CPU::CREATE_SQ, pCPU);

  if (ppSQueue[sqid] == NULL) {
    if (ppCQueue[cqid] != NULL) {
      ppSQueue[sqid] = new SQueue(cqid, priority, sqid, size);
      ppSQueue[sqid]->setBase(
          new PRPList(cfgdata, cpuHandler, pContext, prp1, size * sqstride, pc, pCPU),
          sqstride);
...
void Controller::collectSQueue(DMAFunction &func, void *context) {
  ...
  if (arbitration == ROUND_ROBIN) {
    ...
    while (true) {
      for (uint16_t i = 0; i < sqsize; i++) {
        pQueue = ppSQueue[i];
        bool isCreditorQueue = i >= 2;

        if (pQueue) {
          if (checkQueue(pQueue, doQueue, pContext, isCreditorQueue)) {
            pContext->counter++;
            updated++;
          }
        }
...
void Controller::work() {
  DMAFunction queueFunction = [this](uint64_t now, void *) {
    DMAFunction doRequest = [this](uint64_t, void *) {
      ...
      pCPU->execute(CPU::NVME__CONTROLLER, CPU::HANDLE_REQUEST, handle);
    };
    ...
    pCPU->execute(CPU::NVME__CONTROLLER, CPU::COLLECT_SQ, doRequest);
  };
  ...
  CPUContext *pContext =
      new CPUContext(queueFunction, nullptr, CPU::NVME__CONTROLLER, CPU::WORK, pCPU);
...
void Controller::handleRequest(uint64_t now) {
  ...
  if (lSQFIFO.size() > 0) {
    ...
    if (bUseOCSSD) {
      pCPU->execute(CPU::NVME__OCSSD, CPU::SUBMIT_COMMAND, doSubmit, front);
    }
    else {
      pCPU->execute(CPU::NVME__SUBSYSTEM, CPU::SUBMIT_COMMAND, doSubmit, front);
    }
  }
...
bool Controller::checkQueue(SQueue *pQueue, DMAFunction &func, void *context, bool isCreditorQueue) {
  struct QueueContext {
    SQEntry entry;
    SQueue *pQueue;
    ...
    void *context;
    uint16_t oldhead;
    bool isCreditorQueue;

    QueueContext(DMAFunction &f, bool i) : pQueue(nullptr), function(f), isCreditorQueue(i) {}
  };

  QueueContext *queueContext = new QueueContext(func, isCreditorQueue);
  ...
  DMAFunction doRead = [this](uint64_t now, void *context) {
    ...
    if (pContext->isCreditorQueue) {
      pESPRESSO->remoteReqCnt++;
      lSQFIFO.push_back(SQEntryWrapper(
        pContext->entry, pContext->pQueue->getID(), pContext->pQueue->getCQID(),
        pContext->pQueue->getHead(), pContext->oldhead, true, simpleSolution, pESPRESSO->getDebtorSSDID(pContext->pQueue->getID()), pESPRESSO->ssdID));
    }
    else {
      pESPRESSO->selfReqCnt++;
      lSQFIFO.push_back(SQEntryWrapper(
        pContext->entry, pContext->pQueue->getID(), pContext->pQueue->getCQID(),
...
void Controller::completion() {
  ...
  DMAFunction doSubmit = [this](uint64_t, void *context) {
    ...
    if (pContext->counter == 0) {
      ...
      pCPU->execute(CPU::NVME__CONTROLLER, CPU::COMPLETION, send, pData);
  ...
  for (auto &iter : pData->entryToPost) {
    ...
    // Prevent creditor from needing piggyback
    // if (pESPRESSO->needSetBorrow && pESPRESSO->lending) assert(0);

    // Piggyback ESPRESSO info here
    if (iter.cqID == 1 && pESPRESSO->hasPendingBorrowSetup()) {
      if (iter.entry.dword2.sqID == pESPRESSO->borrowSettingDebtorSQID) {
        uint8_t creditorSSDID = 0;
        if (pESPRESSO->notifyBorrowSetupOnCQ(creditorSSDID)) {
          iter.entry.reserved = 0x80000000U | ((uint32_t)creditorSSDID << 8);
        }
      }
    }
    else if (iter.cqID > 1 && pESPRESSO->hasPendingCancelLend()) {
      if (iter.entry.dword2.sqID == pESPRESSO->peekPendingCancelLendSQID()) {
        uint8_t debtorSSDID = pESPRESSO->peekPendingCancelLendDebtorSSDID();
        iter.entry.reserved = 0x40000000U | ((uint32_t)debtorSSDID << 8);
        pESPRESSO->popPendingCancelLend();
      }
    }
    else if (iter.cqID >= 1) {
      iter.entry.reserved = pESPRESSO->frontBusyValue & 0x000000FFU;
    }
...
SimpleSSD::ESPRESSO::BorrowSettingInfo Controller::getBorrowSettingInfo(uint16_t debtorSQID) {
  return pESPRESSO->getBorrowSettingInfo(debtorSQID);
}

void Controller::updateBorrowingState() {
  pESPRESSO->updateBorrowingState();
}

void Controller::updateLendingState() {
  pESPRESSO->updateLendingState();
}

void Controller::setPending(bool value) {
  pESPRESSO->pending = value;
  pESPRESSO->updatePendingState();
}

void Controller::submitRemoteDMAWrite(uint64_t addr, uint64_t size, uint8_t *buffer, DMAFunction &func, void *context) {
  interconnect->dmaWrite(addr, size, buffer, func, context);
}

void Controller::submitRemoteDMARead(uint64_t addr, uint64_t size, uint8_t *buffer, DMAFunction &func, void *context) {
  interconnect->dmaRead(addr, size, buffer, func, context);
}

void Controller::cancelBorrowOne(uint8_t creditorSSDID) {
  pParent->debtorCancelBorrowOne(creditorSSDID);
}
