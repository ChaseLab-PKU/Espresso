...
class Controller : public StatObject {
 private:
  ...
  bool checkQueue(SQueue *, DMAFunction &, void *, bool=false);

 public:
  ESPRESSO::ESPRESSO *pESPRESSO = nullptr;

  Controller(Interface *, ConfigReader &, CPU::CPU *, uint8_t, bool, std::string);
  ~Controller();
  ...
  int createCQueue(uint16_t, uint16_t, uint16_t, bool, bool, bool, uint64_t,
                   DMAFunction &, void *);
  ...
  SimpleSSD::ESPRESSO::BorrowSettingInfo getBorrowSettingInfo(uint16_t debtorSQID);
  void updateBorrowingState();
  void updateLendingState();
  void setPending(bool);

  void submitRemoteDMAWrite(uint64_t, uint64_t, uint8_t *, DMAFunction &, void *);
  void submitRemoteDMARead(uint64_t, uint64_t, uint8_t *, DMAFunction &, void *);

  void cancelBorrowOne(uint8_t);
