...
OpenChannelSSD12::OpenChannelSSD12(Controller *c, ConfigData &cfg, ESPRESSO::ESPRESSO *pESPRESSO, CPU::CPU *pCPU)
    : Subsystem(c, cfg, pESPRESSO, pCPU),
      pESPRESSO(pESPRESSO),
      pCPU(pCPU),
      lastScheduled(0),
      eraseCount(0),
...
void OpenChannelSSD12::init() {
  ...
  Namespace *pNS = new Namespace(this, cfgdata, pESPRESSO, pCPU);
...
bool OpenChannelSSD12::deviceIdentification(SQEntryWrapper &req,
  ...
  if (req.useSGL) {
    pContext->dma =
        new SGL(cfgdata, doWrite, pContext, req.entry.data1, req.entry.data2, pCPU);
  }
  else {
    pContext->dma = new PRPList(cfgdata, doWrite, pContext, req.entry.data1,
                                req.entry.data2, (uint64_t)0x1000, pCPU);
  }
...
void OpenChannelSSD12::physicalBlockErase(SQEntryWrapper &req,
  ...
  if (!err) {
    ...
    CPUContext *pCPUContext = new CPUContext(doErase, pContext, CPU::NVME__OCSSD,
                                      CPU::PHYSICAL_BLOCK_ERASE, pCPU);

    pContext->beginAt = getTick();

    if (pContext->nlb > 1) {
      pContext->dma = new PRPList(cfgdata, cpuHandler, pCPUContext, pContext->slba,
                                  (uint64_t)nppa * 8, true, pCPU);
    }
    else {
      pContext->dma = nullptr;

      cpuHandler(getTick(), pCPUContext);
    }
  }
...
void OpenChannelSSD12::physicalPageWrite(SQEntryWrapper &req,
  ...
  if (!err) {
    ...
    CPUContext *pCPUContext = new CPUContext(doDMA, pContext, CPU::NVME__OCSSD,
                                      CPU::PHYSICAL_PAGE_WRITE, pCPU);

    pContext->beginAt = getTick();

    if (req.useSGL) {
      pContext->dma =
          new SGL(cfgdata, cpuHandler, pCPUContext, req.entry.data1, req.entry.data2, pCPU);
    }
    else {
      pContext->dma = new PRPList(cfgdata, cpuHandler, pCPUContext, req.entry.data1,
                                  req.entry.data2, (uint64_t)nppa * LBA_SIZE, pCPU);
    }
  }
...
OpenChannelSSD20::OpenChannelSSD20(Controller *c, ConfigData &cfg, ESPRESSO::ESPRESSO *pESPRESSO, CPU::CPU *pCPU)
    : OpenChannelSSD12(c, cfg, pESPRESSO, pCPU),
      pDescriptor(nullptr),
      pESPRESSO(pESPRESSO),
      pCPU(pCPU),
      vectorEraseCount(0),
...
void OpenChannelSSD20::readInternal(std::vector<uint64_t> &lbaList,
  ...
  pContext->beginAt = getTick();
  pCPU->applyLatency(CPU::NVME__OCSSD, CPU::CONVERT_UNIT, pContext->beginAt);
  ...
  pCPU->execute(CPU::NVME__OCSSD, CPU::READ_INTERNAL, doRead, pContext);
}
...
void OpenChannelSSD20::writeInternal(std::vector<uint64_t> &lbaList,
  ...
  pCPU->execute(CPU::NVME__OCSSD, CPU::WRITE_INTERNAL, doWrite, pContext);
}
...
void OpenChannelSSD20::eraseInternal(std::vector<uint64_t> &lbaList,
  ...
  pCPU->execute(CPU::NVME__OCSSD, CPU::ERASE_INTERNAL, doErase, pContext);
}
    ...
    case LOG_CHUNK_INFORMATION: {
      ...
      if (req.useSGL) {
        pContext->dma = new SGL(cfgdata, smartInfo, pContext, req.entry.data1,
                                req.entry.data2, pCPU);
      }
      else {
        pContext->dma =
            new PRPList(cfgdata, smartInfo, pContext, req.entry.data1,
                        req.entry.data2, (uint64_t)req_size, pCPU);
      }
    } break;
    default:
      ret = true;
...
void OpenChannelSSD20::read(SQEntryWrapper &req, RequestFunction &func) {
  ...
  if (!err) {
    ...
    CPUContext *pCPUContext =
        new CPUContext(doRead, pContext, CPU::NVME__OCSSD, CPU::READ, pCPU);
    ...
    if (req.useSGL) {
      ...
      pContext->dma = new PRPList(cfgdata, cpuHandler, pCPUContext, req.entry.data1,
                                  req.entry.data2, pContext->nlb * LBA_SIZE, pCPU);
    }
  }
  else {
    func(resp);
...
void OpenChannelSSD20::write(SQEntryWrapper &req, RequestFunction &func) {
  ...
  if (!err) {
    ...
    CPUContext *pCPUContext =
        new CPUContext(doRead, pContext, CPU::NVME__OCSSD, CPU::WRITE, pCPU);
    ...
    if (req.useSGL) {
      ...
      pContext->dma = new PRPList(cfgdata, cpuHandler, pCPUContext, req.entry.data1,
                                  req.entry.data2, (uint64_t)nlb * LBA_SIZE, pCPU);
    }
  }
  else {
    func(resp);
...
void OpenChannelSSD20::datasetManagement(SQEntryWrapper &req,
  ...
  if (!err) {
    ...
    CPUContext *pCPUContext = new CPUContext(doTrim, pContext, CPU::NVME__OCSSD,
                                      CPU::DATASET_MANAGEMENT, pCPU);
    ...
    if (req.useSGL) {
      ...
      pContext->dma = new PRPList(cfgdata, cpuHandler, pCPUContext, req.entry.data1,
                                  req.entry.data2, (uint64_t)nr * 0x10, pCPU);
    }
  }
  else {
    func(resp);
...
void OpenChannelSSD20::vectorChunkReset(SQEntryWrapper &req,
  ...
  CPUContext *pCPUContext = new CPUContext(doRead, pContext, CPU::NVME__OCSSD,
                                    CPU::VECTOR_CHUNK_RESET, pCPU);
