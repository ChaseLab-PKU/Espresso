...
PRPList::PRPList(ConfigData &cfg, DMAFunction &f, void *c, uint64_t prp1,
                 uint64_t prp2, uint64_t size, CPU::CPU *pCPU)
    : DMAInterface(cfg, f, c), pCPU(pCPU), totalSize(size), pagesize(cfg.memoryPageSize) {
  bool immediate = true;
...
PRPList::PRPList(ConfigData &cfg, DMAFunction &f, void *c, uint64_t base,
                 uint64_t size, bool cont, CPU::CPU *pCPU)
    : DMAInterface(cfg, f, c), pCPU(pCPU), totalSize(size), pagesize(cfg.memoryPageSize) {
  if (cont) {
...
void PRPList::getPRPListFromPRP(uint64_t base, uint64_t size) {
  ...
  if (pContext->buffer) {
    // Read PRP
    CPUContext *pCPUContext = new CPUContext(doRead, pContext, CPU::NVME__PRPLIST,
                                      CPU::GET_PRPLIST_FROM_PRP, pCPU);
    pInterface->dmaRead(base, pContext->currentSize, pContext->buffer,
                        cpuHandler, pCPUContext);
  }
  else {
    delete pContext;
...
void PRPList::read(uint64_t offset, uint64_t length, uint8_t *buffer,
                   DMAFunction &func, void *context, bool isDataDMA) {
  DMAFunction doRead = [this, offset, length, buffer, isDataDMA](uint64_t, void *context) {
    ...
    for (auto &iter : prpList) {
      if (begin) {
        ...
        if (isDataDMA) {
          IOContext *reqContext = (IOContext *)(readContext->context);
          if (reqContext->isRemoteReq && !reqContext->isSimpleSolution) {
            pESPRESSO->submitRemoteDMARead(reqContext->debtorSSDID, iter.addr, read, buffer ? buffer + totalRead : NULL, dmaHandler, readContext);
          }
          else pInterface->dmaRead(iter.addr, read, buffer ? buffer + totalRead : NULL, dmaHandler, readContext);
        }
        else pInterface->dmaRead(iter.addr, read, buffer ? buffer + totalRead : NULL, dmaHandler, readContext);
      ...
      if (!begin && currentOffset + iter.size > offset) {
        ...
        if (isDataDMA) {
          IOContext *reqContext = (IOContext *)(readContext->context);
          if (reqContext->isRemoteReq && !reqContext->isSimpleSolution) {
            pESPRESSO->submitRemoteDMARead(reqContext->debtorSSDID, iter.addr + totalRead, read, buffer, dmaHandler,readContext);
          }
          else pInterface->dmaRead(iter.addr + totalRead, read, buffer, dmaHandler,readContext);
        }
        else pInterface->dmaRead(iter.addr + totalRead, read, buffer, dmaHandler,readContext);
  ...
  pCPU->execute(CPU::NVME__PRPLIST, CPU::READ, doRead, readContext);
}

void PRPList::write(uint64_t offset, uint64_t length, uint8_t *buffer,
                    DMAFunction &func, void *context, bool isDataDMA) {
  DMAFunction doWrite = [this, offset, length, buffer, isDataDMA] (uint64_t, void *context) {
    ...
    for (auto &iter : prpList) {
      if (begin) {
        ...
        if (isDataDMA) {
          IOContext *reqContext = (IOContext *)(writeContext->context);
          if (reqContext->isRemoteReq && !reqContext->isSimpleSolution) {
            pESPRESSO->submitRemoteDMAWrite(reqContext->debtorSSDID, iter.addr, written, buffer ? buffer + totalWritten : NULL, dmaHandler, writeContext);
          }
          else pInterface->dmaWrite(iter.addr, written, buffer ? buffer + totalWritten : NULL, dmaHandler, writeContext);
        }
        else pInterface->dmaWrite(iter.addr, written, buffer ? buffer + totalWritten : NULL, dmaHandler, writeContext);
      ...
      if (!begin && currentOffset + iter.size > offset) {
        ...
        if (isDataDMA) {
          IOContext *reqContext = (IOContext *)(writeContext->context);
          if (reqContext->isRemoteReq && !reqContext->isSimpleSolution) {
            pESPRESSO->submitRemoteDMAWrite(reqContext->debtorSSDID, iter.addr + totalWritten, written, buffer, dmaHandler, writeContext);
          }
          else pInterface->dmaWrite(iter.addr + totalWritten, written, buffer, dmaHandler, writeContext);
        }
        else pInterface->dmaWrite(iter.addr + totalWritten, written, buffer, dmaHandler, writeContext);
  ...
  pCPU->execute(CPU::NVME__PRPLIST, CPU::WRITE, doWrite, writeContext);
}
...
SGL::SGL(ConfigData &cfg, DMAFunction &f, void *c, uint64_t prp1, uint64_t prp2, CPU::CPU *pCPU)
    : DMAInterface(cfg, f, c), pCPU(pCPU), totalSize(0) {
  SGLDescriptor desc;
...
void SGL::parseSGLSegment(uint64_t address, uint32_t length) {
  ...
  if (pContext->buffer) {
    // Read segment
    CPUContext *pCPUContext = new CPUContext(doRead, pContext, CPU::NVME__SGL,
                                      CPU::PARSE_SGL_SEGMENT, pCPU);
    pInterface->dmaRead(address, length, pContext->buffer, cpuHandler, pCPUContext);
  }
  else {
    delete pContext;
...
void SGL::read(uint64_t offset, uint64_t length, uint8_t *buffer,
  ...
  pCPU->execute(CPU::NVME__SGL, CPU::READ, doRead, readContext);
}

void SGL::write(uint64_t offset, uint64_t length, uint8_t *buffer,
                DMAFunction &func, void *context, bool isDataDMA) {
  DMAFunction doWrite = [this, offset, length, buffer, isDataDMA](uint64_t,
                                                       void *context) {
    uint64_t totalWritten = 0;
    uint64_t currentOffset = 0;
    uint64_t written;
  ...
  pCPU->execute(CPU::NVME__SGL, CPU::WRITE, doWrite, writeContext);
}
