...
SQEntryWrapper::_SQEntryWrapper(SQEntry &sqdata, uint16_t sqid, uint16_t cqid,
                                uint16_t sqhead, uint16_t squid, bool isRemoteReq, bool isSimpleSolution, uint8_t debtorSSDID, uint8_t creditorSSDID)
    : entry(sqdata),
      sqID(sqid),
      cqID(cqid),
      sqHead(sqhead),
      sqUID(squid),
      useSGL(true),
      isRemoteReq(isRemoteReq),
      isSimpleSolution(isSimpleSolution),
      debtorSSDID(debtorSSDID),
      creditorSSDID(creditorSSDID) {
...
CQEntryWrapper::_CQEntryWrapper(SQEntryWrapper &sqew) {
  ...
  isRemoteReq = sqew.isRemoteReq;
  isSimpleSolution = sqew.isSimpleSolution;
  debtorSSDID = sqew.debtorSSDID;
  creditorSSDID = sqew.creditorSSDID;
}
...
CQueue::CQueue(uint16_t iv, bool is, bool en, uint16_t qid, uint16_t size)
    : Queue(qid, size), isCreditorQueue(is), ien(en), phase(true), interruptVector(iv) {}
