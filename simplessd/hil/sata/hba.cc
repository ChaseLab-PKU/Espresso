...
HBA::HBA(Interface *i, CPU::CPU *pCPU, ConfigReader &c) : pInterface(i), pCPU(pCPU), conf(c) {
  ...
  pDevice = new Device(this, pDeviceDMA, pCPU, conf);
}
...
void HBA::handleRequest() {
  ...
  if (lRequestQueue.size() > 0) {
    ...
    CPUContext *pCPUContext = new CPUContext(doRead, pContext, CPU::SATA__DEVICE,
                                      CPU::SUBMIT_COMMAND, pCPU);
