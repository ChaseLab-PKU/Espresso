...
Device::Device(HBA *p, DMAInterface *d, CPU::CPU *pCPU, ConfigReader &c)
    : pParent(p), pDMA(d), pDisk(nullptr), pCPU(pCPU), conf(c) {
  pHIL = new HIL(c, pCPU);
...
void Device::prdtRead(uint8_t *prdt, uint32_t prdtLength, uint32_t length,
  ...
  pCPU->execute(CPU::SATA__DEVICE, CPU::PRDT_READ, doRead, pContext);
}
...
void Device::prdtWrite(uint8_t *prdt, uint32_t prdtLength, uint32_t length,
  ...
  pCPU->execute(CPU::SATA__DEVICE, CPU::PRDT_WRITE, doWrite, pContext);
}
...
void Device::read(uint64_t slba, uint64_t nlb, uint8_t *buffer,
  ...
  pCPU->execute(CPU::SATA__DEVICE, CPU::READ, doRead, req);
}
...
void Device::write(uint64_t slba, uint64_t nlb, uint8_t *buffer,
  ...
  pCPU->execute(CPU::SATA__DEVICE, CPU::WRITE, doWrite, req);
}
...
void Device::flush(DMAFunction &func, void *context) {
  ...
  pCPU->execute(CPU::SATA__DEVICE, CPU::FLUSH, doFlush, req);
}
...
void Device::_writeDMASetup(uint64_t, void *context) {
  ...
  // Begin DMA
  CPUContext *pCPUContext = new CPUContext(writeDMADone, pContext, CPU::SATA__DEVICE,
                                    CPU::WRITE_DMA_SETUP, pCPU);

  prdtRead(pContext->prdt, pContext->prdtLength, pContext->nlb * lbaSize,
           pContext->buffer, cpuHandler, pCPUContext);
}
...
void Device::_writeDMADone(uint64_t tick, void *context) {
  ...
  CPUContext *pCPUContext = new CPUContext(nandDone, pContext, CPU::SATA__DEVICE,
                                    CPU::WRITE_DMA_DONE, pCPU);

  write(pContext->slba, pContext->nlb, pContext->buffer, cpuHandler, pCPUContext);
}
...
void Device::_readDMASetup(uint64_t tick, void *context) {
  ...
  // Begin DMA
  CPUContext *pCPUContext = new CPUContext(readDMADone, pContext, CPU::SATA__DEVICE,
                                    CPU::READ_DMA_SETUP, pCPU);
...
void Device::_readDMADone(uint64_t tick, void *context) {
  ...
  pCPU->execute(CPU::SATA__DEVICE, CPU::READ_DMA_DONE, doSubmit, pContext);
}
...
void Device::readDMA(CommandContext *cmd, bool isPIO) {
  ...
  CPUContext *pCPUContext =
      new CPUContext(doRead, pContext, CPU::SATA__DEVICE, CPU::READ_DMA, pCPU);

  read(slba, nlb, pContext->buffer, cpuHandler, pCPUContext);
}
...
void Device::readNCQ(CommandContext *cmd) {
  ...
  pCPU->execute(CPU::SATA__DEVICE, CPU::READ_NCQ, doSubmit, cmd);
}
...
void Device::writeDMA(CommandContext *cmd, bool isPIO) {
  ...
  CPUContext *pCPUContext =
      new CPUContext(doRead, pContext, CPU::SATA__DEVICE, CPU::WRITE_DMA, pCPU);
...
void Device::writeNCQ(CommandContext *cmd) {
  ...
  pCPU->execute(CPU::SATA__DEVICE, CPU::WRITE_NCQ, doSubmit, wrapper);
}
