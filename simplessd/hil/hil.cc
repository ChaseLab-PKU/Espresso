...
HIL::HIL(ConfigReader &c, CPU::CPU *pCPU, ESPRESSO::ESPRESSO *pESPRESSO) : conf(c), pCPU(pCPU), pESPRESSO(pESPRESSO), reqCount(0), lastScheduled(0) {
  pESPRESSO->setHIL(this);

  pICL = new ICL::ICL(conf, pCPU, pESPRESSO);
...
void HIL::updateSubmit() {
  DMAFunction doRequest = [this](uint64_t beginAt, void *context) {
    ...
    if (pReq->reqType != READ && pReq->reqType != WRITE && pReq->isRemoteReq) assert(0);
    ...
    switch (pReq->reqType) {
      case READ:
        ...
        // ESFUTUREWORK: updateBustTime do nothing in current implementation
  ...
  while (submitQueue.size() > 0 && !pICL->submitQueueIsFull()) {
    ...
    pCPU->execute(CPU::HIL, func, doRequest, new Request(req));
  }
}
