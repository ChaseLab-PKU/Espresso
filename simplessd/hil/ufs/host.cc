...
Host::Host(Interface *i, CPU::CPU *pCPU, ConfigReader &c)
    : pInterface(i),
      pCPU(pCPU),
      conf(c),
      requestCounter(0),
  ...
  pDevice = new Device(deviceFIFO, pCPU, c);
  ...
  DMAFunction doRequest = [this](uint64_t now, void *context) {
    ...
    switch (reqType) {
      ...
      case OPCODE_QUERY_REQUEST:
        ...
        pCPU->execute(CPU::UFS__DEVICE, CPU::PROCESS_QUERY_COMMAND, done, context);
      ...
      case OPCODE_COMMAND:
        ...
        {
          CPUContext *pCPUContext = new CPUContext(done, context, CPU::UFS__DEVICE,
                                            CPU::PROCESS_COMMAND, pCPU);
          ...
          pDevice->processCommand(
              ...
              pResult->transferReqDesc.prdtLength, cpuHandler, pCPUContext);
        }
