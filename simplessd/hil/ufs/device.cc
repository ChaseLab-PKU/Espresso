...
Device::Device(DMAInterface *d, CPU::CPU *pCPU, ConfigReader &c)
    : pDMA(d),
      pHIL(nullptr),
      pDisk(nullptr),
      pCPU(pCPU),
...
void Device::prdtRead(uint8_t *prdt, uint32_t prdtLength, uint32_t length,
  ...
  pCPU->execute(CPU::UFS__DEVICE, CPU::PRDT_READ, doRead, pContext);
}
...
void Device::prdtWrite(uint8_t *prdt, uint32_t prdtLength, uint32_t length,
  ...
  pCPU->execute(CPU::UFS__DEVICE, CPU::PRDT_WRITE, doWrite, pContext);
}
...
void Device::read(uint64_t slba, uint64_t nlb, uint8_t *buffer,
  ...
  pCPU->execute(CPU::UFS__DEVICE, CPU::READ, doRead, req);
}
...
void Device::write(uint64_t slba, uint64_t nlb, uint8_t *buffer,
  ...
  pCPU->execute(CPU::UFS__DEVICE, CPU::WRITE, doWrite, req);
}
...
void Device::flush(DMAFunction &func, void *context) {
  ...
  pCPU->execute(CPU::UFS__DEVICE, CPU::FLUSH, doFlush, req);
}
