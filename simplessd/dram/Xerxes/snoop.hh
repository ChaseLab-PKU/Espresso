...
class Snoop : public Device {
  class SnoopEviction {
   ...
   public:
    ...
    virtual void on_hit(Addr, size_t, size_t) {}
    virtual void on_update(Addr, size_t, size_t) {}
    virtual void on_insert(Addr, size_t, size_t) {}
    virtual void on_invalidate(Addr, size_t, size_t) {}
    virtual void on_evict(Addr, size_t, size_t) {}
    virtual ssize_t find_victim(size_t, bool) = 0;
  };
  ...
  class FIFO : public SnoopEviction {
   ...
   public:
    ...
    void on_invalidate(Addr, size_t set_i, size_t way_i) override {
      auto &q = queues[set_i];
    ...
    void on_insert(Addr, size_t set_i, size_t way_i) override {
      bool exist = false;
  ...
  class LRU : public FIFO {
    ...
    */
    ...
    void on_hit(Addr, size_t set_i, size_t way_i) override {
      auto &q = queues[set_i];
      for (auto it = q.begin(); it != q.end(); ++it) {
        if (*it == way_i) {
          q.erase(it);
    ...
    void on_update(Addr, size_t set_i, size_t way_i) override {
      auto &q = queues[set_i];
      for (auto it = q.begin(); it != q.end(); ++it) {
        if (*it == way_i) {
          q.erase(it);
  ...
  std::unordered_set<TopoID> snooping;

  size_t set_of(Addr addr) { return (addr / 64) % set_num; }

  ssize_t hit(Addr addr, TopoID) {
  ...
  void filter(Packet pkt) {
    if (pkt.is_coherent() && !pkt.is_rsp &&
        snooping.find(pkt.dst) != snooping.end()) {
      coherent_request(pkt);
    }
    else if (pkt.type == PacketType::INVALID && pkt.is_rsp && pkt.dst == self) {
 ...
 public:
  Snoop(Simulation *sim, const SnoopConfig &config, std::string name = "Snoop")
    ...
    for (auto &c : cache)
      c.resize(assoc, Line{0, false, INVALID, false});
    ...
    snooping = std::unordered_set<TopoID>{};
  }

  ~Snoop() {}

  void add_snooping_id(TopoID id) { snooping.insert(id); }
