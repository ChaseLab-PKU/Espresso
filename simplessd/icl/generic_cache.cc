...
GenericCache::GenericCache(ConfigReader &c, FTL::FTL *f, DRAM::AbstractDRAM *d, CPU::CPU *pCPU, ESPRESSO::ESPRESSO *pESPRESSO)
    : AbstractCache(c, f, d, pESPRESSO),
      pCPU(pCPU),
  ...
  while (pFTL->commitToUpper(ftlReq)) {
    ...
    if (!find) {
      ...
      if(ftlReq.reqType == READ && useReadCaching){
        if (ftlReq.isRemoteReq && !ftlReq.isSimpleSolution) {
          pESPRESSO->conductRemoteDRAMWrite(ftlReq.debtorSSDID, &cacheData[ctxt->setIdx][ctxt->wayIdx], ctxt->req.length, ctxt->req.finishAt);
        }
        else pDRAM->write(&cacheData[ctxt->setIdx][ctxt->wayIdx], ctxt->req.length, ctxt->req.finishAt);
...
void GenericCache::evictCache(Request *req, cacheReqContext *context, uint64_t &ftlSubID, uint64_t tick,
  ...
  for (uint32_t row = 0; row < lineCountInSuperPage; row++) {
    ...
    for (uint32_t idx = 0; idx < mappingUnitInLine; idx++) {
      ...
      if (useWriteCaching) 
        evictinfo_insert(*req, evictData[i]);
...
void GenericCache::read(Request &req, uint64_t &tick){
  ...
  if (useReadCaching) {
    // ESFUTUREWORK: we do not support cache now
    assert(req.isRemoteReq == false);
    ...
    context.steps.push_back(REQFUNC {
      ...
      if (c->req.isRemoteReq && !c->req.isSimpleSolution) {
        pESPRESSO->conductRemoteDRAMWrite(c->req.debtorSSDID, nullptr, c->req.length, tick);
      }
      else pDRAM->write(nullptr, c->req.length, tick);
...
bool GenericCache::readInternal(Request &req, uint64_t &tick) {
  ...
  bool prefetch = false;
  if (useReadPrefetch) {
    // ESFUTUREWORK: we do not support prefetch now
    assert(req.isRemoteReq == false);
  ...
  context.steps.push_back(REQFUNC {
    ...
    if (c->hit) {
      ...
      if (!pl->waiting) {
        ...
        if (c->req.isRemoteReq && !c->req.isSimpleSolution) {
          pESPRESSO->conductRemoteDRAMRead(c->req.debtorSSDID, pl, c->req.length, hitTick);
        }
        else pDRAM->read(pl, c->req.length, hitTick);
  ...
  context.steps.push_back(REQFUNC {
    ...
    uint64_t tick = beginAt;
    pCPU->applyLatency(CPU::ICL__GENERIC_CACHE, CPU::READ, tick);
  ...
  if(useWriteCaching){
   ...
   /*if (req.length <= lineSize * mappingUnitInLine ||
    ...
    context.steps.push_back(REQFUNC {
      ...
      if(c->req.isRemoteReq && !c->req.isSimpleSolution) { 
        pESPRESSO->conductRemoteDRAMRead(c->req.debtorSSDID, nullptr, c->req.length, tick);
      }
      else pDRAM->read(nullptr, c->req.length, tick);
...
bool GenericCache::writeInternal(Request &req, uint64_t &tick, bool fromRead = false) {
  ...
  context.steps.push_front(REQFUNC {
    ...
    if (c->req.isRemoteReq && !c->req.isSimpleSolution) {
      pESPRESSO->conductRemoteDRAMWrite(c->req.debtorSSDID, &cacheData[s][w], c->req.length, tick);
    }
    else pDRAM->write(&cacheData[s][w], c->req.length, tick);
  ...
  context.steps.push_back(REQFUNC {
    uint64_t tick = beginAt;
    pCPU->applyLatency(CPU::ICL__GENERIC_CACHE, CPU::WRITE, tick);
    c->req.finishAt = MAX(c->req.finishAt, tick);
    return false;
  }); 
  // clean up works
...
void GenericCache::flush(Request &req, uint64_t &tick) {
  ...
  context.steps.push_back(REQFUNC {
    pCPU->applyLatency(CPU::ICL__GENERIC_CACHE, CPU::FLUSH, beginAt);
...
void GenericCache::trim(Request &req, uint64_t &tick) {
  ...
  context.steps.push_back(REQFUNC {
    pCPU->applyLatency(CPU::ICL__GENERIC_CACHE, CPU::TRIM, beginAt);
    c->req.finishAt = beginAt;
    return false;
  });
}
...
void GenericCache::format(Request &req, uint64_t &tick) {
  ...
  context.steps.push_back(REQFUNC {
    pCPU->applyLatency(CPU::ICL__GENERIC_CACHE, CPU::FORMAT, beginAt);
    c->req.finishAt = beginAt;
    return false;
  });
