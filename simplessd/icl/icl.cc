...
ICL::ICL(ConfigReader &c, CPU::CPU *pCPU, ESPRESSO::ESPRESSO *pESPRESSO) : pCPU(pCPU), pESPRESSO(pESPRESSO), conf(c) {
  pESPRESSO->setICL(this);

  switch (conf.readInt(CONFIG_DRAM, DRAM::DRAM_MODEL)) {
    case DRAM::SIMPLE_MODEL:
      pDRAM = new DRAM::SimpleDRAM(conf);
      pESPRESSO->setDRAM(pDRAM);
  ...
  pFTL = new FTL::FTL(conf, pDRAM, pCPU, pESPRESSO);
  ...
  pCache = new GenericCache(conf, pFTL, pDRAM, pCPU, pESPRESSO);
...
void ICL::updateSubmit() {
  ...
  for (auto iter = submitQueue.begin(); iter != submitQueue.end(); ++iter) {
    ...
    pCPU->execute(CPU::ICL, func, doRequest, new Request(iter->first));
  }
}
