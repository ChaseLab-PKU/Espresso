...
const char SECTION_ESPRESSO[] = "espresso";
...
bool ConfigReader::init(std::string file, bool host) {
...
int64_t ConfigReader::readInt(CONFIG_SECTION section, uint32_t idx) {
  switch (section) {
    ...
    case CONFIG_ESPRESSO:
      return espressoConfig.readInt(idx);
    default:
      return 0;
  }
}
...
uint64_t ConfigReader::readUint(CONFIG_SECTION section, uint32_t idx) {
  switch (section) {
    ...
    case CONFIG_ESPRESSO:
      return espressoConfig.readUint(idx);
    default:
      return 0;
  }
}
...
float ConfigReader::readFloat(CONFIG_SECTION section, uint32_t idx) {
  switch (section) {
    ...
    case CONFIG_ESPRESSO:
      return espressoConfig.readFloat(idx);
    default:
      return 0.f;
  }
}
...
std::string ConfigReader::readString(CONFIG_SECTION section, uint32_t idx) {
  switch (section) {
    ...
    case CONFIG_ESPRESSO:
      return espressoConfig.readString(idx);
    default:
      return std::string();
  }
}
...
bool ConfigReader::readBoolean(CONFIG_SECTION section, uint32_t idx) {
  switch (section) {
    ...
    case CONFIG_ESPRESSO:
      return espressoConfig.readBoolean(idx);
    default:
      return false;
  }
}
...
int ConfigReader::parserHandler(void *context, const char *section,
  ...
  else if (MATCH_SECTION(SECTION_ESPRESSO)) {
    handled = pThis->espressoConfig.setConfig(name, value);
  }
