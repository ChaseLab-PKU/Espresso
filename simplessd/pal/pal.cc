...
PAL::PAL(ConfigReader &c, ESPRESSO::ESPRESSO *pESPRESSO) : conf(c), pESPRESSO(pESPRESSO) {
  pESPRESSO->setPAL(this);
...
void PAL::commitRemotePALReq(Request &req) {
  commitQueue.push_back(req);
}

double PAL::checkBusy() {
  return pPAL->checkBusy();
}
}  // namespace PAL
