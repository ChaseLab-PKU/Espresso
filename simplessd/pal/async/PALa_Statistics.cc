...
double ChannelStatistics::checkBusy() {
  uint64_t now = SimpleSSD::getTick();

  double caBusyRatio = (double) (cabusy - lastCheckCABusy) / (now - lastCheckTick);
  double dieBusyRatio = (double) (dietotalbusy - lastCheckDieBusy) / numDies / (now - lastCheckTick);
  double busyRatio = std::max(caBusyRatio, dieBusyRatio);

  lastCheckTick = now;
  lastCheckCABusy = cabusy;
  lastCheckDieBusy = dietotalbusy;
...
void ChannelStatistics::getStatList(std::vector<SimpleSSD::Stats> &list,
  ...
  // BUG: when EnableSCA = 0, cabus actually counts databus traffic
  temp.name = prefix + "cabus_busy";
  temp.desc = "Busy time of Cmd/Addr bus";
  list.push_back(temp);

  temp.name = prefix + "data_bus_busy";
  temp.desc = "Busy time of Data bus";
  list.push_back(temp);

  temp.name = prefix + "die_busy";
  temp.desc = "Busy time of dies";
  list.push_back(temp);
...
void ChannelStatistics::getStatValues(std::vector<double> &values) {
  // ca bus busy
  values.push_back((double)cabusy);
  // data bus busy
  values.push_back((double)dbusy);
  // die busy
  values.push_back((double)dietotalbusy / numDies);
  // energy.read
