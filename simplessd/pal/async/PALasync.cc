...
double PALasync::checkBusy() {
  double palBusyRatio = 0;
  double channelBusyRatio = 0;

  for (auto &ch : channels) {
    ch.flushTimeslot(SimpleSSD::getTick(), nullptr);
  }

  for(auto &ch : channels) {
    channelBusyRatio = ch.checkBusy();
    palBusyRatio = std::max(channelBusyRatio, palBusyRatio);
  }
