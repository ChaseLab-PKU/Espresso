...
class CPU : public StatObject {
 private:
  ...
  std::vector<Core> espressoCore;
  
  std::vector<Core> nvmeCore;
  std::vector<Core> generalCore;
  

  // CPIs
  std::unordered_map<uint16_t, std::unordered_map<uint16_t, InstStat>> cpi;

  uint64_t espressoCPUBusy = 0;
