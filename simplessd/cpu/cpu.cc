...
InstStat::_InstStat(uint64_t b, uint64_t l, uint64_t s, uint64_t a, uint64_t f,
    ...
    : branch(b),
      ...
      otherInsts(o) {
  latency = sum();
  // latency *= 1.1;  // Add some overhead to mimic the overhead of synchronization
  latency *= c;
}
...
double CPU::Core::checkBusy() {
  uint64_t now = getTick();

  double busyRatio = (double) (stat.busy - lastCheckBusy) / (now - lastCheckTick);

  lastCheckTick = now;
  lastCheckBusy = stat.busy;
...
CPU::CPU(ConfigReader &c, uint8_t id) : conf(c), ssdID(id), lastResetStat(0) {
  ...
  mode = conf.readUint(CONFIG_CPU, CPU_MODE);
  ...
  if (mode == 0) {
    ...
    espressoCore.resize(conf.readUint(CONFIG_CPU, CPU_CORE_ESPRESSO));
  }
  else if (mode == 1) {
    nvmeCore.resize(conf.readUint(CONFIG_CPU, CPU_CORE_NVME));
    generalCore.resize(conf.readUint(CONFIG_CPU, CPU_CORE_GENERAL));
    eccCore.resize(conf.readUint(CONFIG_CPU, CPU_CORE_ECC));
  }
  ...
  cpi.insert({ESPRESSO, std::unordered_map<uint16_t, InstStat>()});

  // Insert item (Use cpu/generator/generate.py to generate this code)
  cpi.find(0)->second.insert({0, InstStat(8, 84, 21, 15, 0, 0, clockPeriod)});
  ...
  cpi.find(1)->second.insert({0, InstStat(15, 112, 17, 58, 0, 1, clockPeriod)});
  cpi.find(1)->second.insert({1, InstStat(13, 100, 15, 68, 0, 0, clockPeriod)});
  cpi.find(1)->second.insert({3, InstStat(3, 32, 6, 12, 0, 2, clockPeriod)});
  cpi.find(1)->second.insert({4, InstStat(51, 148, 24, 103, 0, 0, clockPeriod)});
  cpi.find(1)->second.insert({9, InstStat(157, 508, 93, 284, 105, 8, clockPeriod)});
  cpi.find(1)->second.insert({10, InstStat(195, 1264, 235, 406, 0, 3, clockPeriod)});
  cpi.find(1)->second.insert({5, InstStat(95, 460, 79, 185, 0, 2, clockPeriod)});
  cpi.find(1)->second.insert({6, InstStat(173, 892, 113, 398, 43, 3, clockPeriod)});
  cpi.find(1)->second.insert({8, InstStat(28, 188, 29, 70, 0, 2, clockPeriod)});
  cpi.find(1)->second.insert({7, InstStat(40, 148, 35, 73, 0, 1, clockPeriod)});
  ...
  cpi.find(4)->second.insert({0, InstStat(21, 136, 47, 39, 0, 0, clockPeriod)});
  cpi.find(4)->second.insert({1, InstStat(21, 136, 47, 39, 0, 0, clockPeriod)}); 
  cpi.find(4)->second.insert({2, InstStat(21, 136, 47, 39, 0, 0, clockPeriod)});
  ...
  cpi.find(8)->second.insert({19, InstStat(123, 220, 52, 170, 0, 6, clockPeriod)});
  cpi.find(8)->second.insert({20, InstStat(4, 40, 14, 112, 0, 0, clockPeriod)});
  cpi.find(8)->second.insert({21, InstStat(73, 208, 51, 173, 0, 2, clockPeriod)});
  ...
  cpi.find(10)->second.insert(
      {0, InstStat(98, 376, 78, 168, 0, 3, clockPeriod)});
  cpi.find(10)->second.insert(
      {1, InstStat(99, 364, 76, 172, 0, 6, clockPeriod)});
  cpi.find(10)->second.insert(
      {22, InstStat(125, 428, 96, 217, 0, 9, clockPeriod)});
  cpi.find(10)->second.insert(
      {5, InstStat(57, 228, 71, 99, 0, 5, clockPeriod)});
  cpi.find(10)->second.insert(
      {6, InstStat(74, 292, 79, 148, 0, 5, clockPeriod)});
  cpi.find(10)->second.insert(
      {7, InstStat(70, 260, 79, 123, 0, 5, clockPeriod)});
  cpi.find(10)->second.insert(
      {20, InstStat(52, 300, 47, 244, 0, 1, clockPeriod)});
  cpi.find(10)->second.insert(
      {23, InstStat(128, 468, 97, 226, 0, 8, clockPeriod)});
  cpi.find(10)->second.insert(
      {24, InstStat(130, 488, 98, 231, 0, 9, clockPeriod)});
  cpi.find(10)->second.insert(
      {25, InstStat(72, 240, 58, 129, 0, 2, clockPeriod)});
  cpi.find(10)->second.insert(
      {26, InstStat(134, 468, 93, 233, 0, 7, clockPeriod)});
  cpi.find(10)->second.insert(
      {27, InstStat(149, 508, 104, 257, 0, 9, clockPeriod)});
  cpi.find(10)->second.insert(
      {28, InstStat(121, 388, 81, 211, 0, 10, clockPeriod)});
  ...
  cpi.find(14)->second.insert({43, InstStat(41, 161, 30, 30, 0, 20, clockPeriod)});
  cpi.find(14)->second.insert({44, InstStat(21, 116, 26, 43, 0, 5, clockPeriod)});
  cpi.find(14)->second.insert({45, InstStat(19, 76, 11, 32, 0, 5, clockPeriod)});
  cpi.find(14)->second.insert({46, InstStat(5, 5, 4, 7, 0, 0, clockPeriod)});
  cpi.find(14)->second.insert({47, InstStat(95, 460, 79, 185, 0, 2, clockPeriod)});
  cpi.find(14)->second.insert({48, InstStat(173, 892, 113, 398, 43, 3, clockPeriod)});
  cpi.find(14)->second.insert({49, InstStat(95, 460, 79, 185, 0, 2, clockPeriod)});
}

CPU::~CPU() {
  std::cout << "CPU Espresso:" << espressoCPUBusy << std::endl;
}
...
void CPU::calculatePower(Power &power) {
  ...
  uint32_t totalCore = hilCore.size() + iclCore.size() + ftlCore.size() + eccCore.size() + espressoCore.size() + 
                       nvmeCore.size() + generalCore.size();
  uint32_t coreIdx = 0;
...
void CPU::execute(NAMESPACE ns, FUNCTION fct, DMAFunction &func, void *context,
  ...
  if (mode == 0) {
    switch (ns) {
      ...
      case SATA__DEVICE:
        ...
        break;
      case ESPRESSO:
        if (espressoCore.size() > 0) {
          pCore = &espressoCore.at(leastBusyCPU(espressoCore));
        }
    ...
    switch (ns) {
      ...
      case UFS__DEVICE:
      case SATA__DEVICE:
        if (generalCore.size() > 0) {
          pCore = &generalCore.at(leastBusyCPU(generalCore));
        }
  ...
  if (pCore) {
    ...
    if (ns == ESPRESSO) {
      espressoCPUBusy += inst->second.latency;
    }
...
double CPU::checkBusy() {
  double cpuBusyRatio = 0;
  double coreBusyRatio = 0;

  for (auto &core : hilCore) {
    coreBusyRatio = core.checkBusy();
    cpuBusyRatio = std::max(coreBusyRatio, cpuBusyRatio);
  }
...
void CPU::getStatList(std::vector<Stats> &list, std::string prefix) {
  ...
  for (uint32_t i = 0; i < espressoCore.size(); i++) {
    number = std::to_string(i);

    temp.name = prefix + ".espresso" + number + ".busy";
    temp.desc = "CPU for ESPRESSO core " + number + " busy ticks";
    list.push_back(temp);

    temp.name = prefix + ".espresso" + number + ".insts.branch";
    temp.desc = "CPU for ESPRESSO core " + number + " executed branch instructions";
    list.push_back(temp);

    temp.name = prefix + ".espresso" + number + ".insts.load";
    temp.desc = "CPU for ESPRESSO core " + number + " executed store instructions";
    list.push_back(temp);

    temp.name = prefix + ".espresso" + number + ".insts.store";
    temp.desc = "CPU for ESPRESSO core " + number + " executed load instructions";
    list.push_back(temp);

    temp.name = prefix + ".espresso" + number + ".insts.arithmetic";
    temp.desc =
        "CPU for ESPRESSO core " + number + " executed arithmetic instructions";
    list.push_back(temp);

    temp.name = prefix + ".espresso" + number + ".insts.fp";
    temp.desc =
        "CPU for ESPRESSO core " + number + " executed floating point instructions";
    list.push_back(temp);

    temp.name = prefix + ".espresso" + number + ".insts.others";
    temp.desc = "CPU for ESPRESSO core " + number + " executed other instructions";
    list.push_back(temp);
  }

  for (uint32_t i = 0; i < nvmeCore.size(); i++) {
    number = std::to_string(i);

    temp.name = prefix + ".nvme" + number + ".busy";
    temp.desc = "CPU for NVMe core " + number + " busy ticks";
    list.push_back(temp);

    temp.name = prefix + ".nvme" + number + ".insts.branch";
    temp.desc = "CPU for NVMe core " + number + " executed branch instructions";
    list.push_back(temp);

    temp.name = prefix + ".nvme" + number + ".insts.load";
    temp.desc = "CPU for NVMe core " + number + " executed store instructions";
    list.push_back(temp);

    temp.name = prefix + ".nvme" + number + ".insts.store";
    temp.desc = "CPU for NVMe core " + number + " executed load instructions";
    list.push_back(temp);

    temp.name = prefix + ".nvme" + number + ".insts.arithmetic";
    temp.desc =
        "CPU for NVMe core " + number + " executed arithmetic instructions";
    list.push_back(temp);

    temp.name = prefix + ".nvme" + number + ".insts.fp";
    temp.desc =
        "CPU for NVMe core " + number + " executed floating point instructions";
    list.push_back(temp);

    temp.name = prefix + ".nvme" + number + ".insts.others";
    temp.desc = "CPU for NVMe core " + number + " executed other instructions";
    list.push_back(temp);
  }

  for (uint32_t i = 0; i < generalCore.size(); i++) {
    number = std::to_string(i);

    temp.name = prefix + ".general" + number + ".busy";
    temp.desc = "CPU for general core " + number + " busy ticks";
    list.push_back(temp);

    temp.name = prefix + ".general" + number + ".insts.branch";
    temp.desc = "CPU for general core " + number + " executed branch instructions";
    list.push_back(temp);

    temp.name = prefix + ".general" + number + ".insts.load";
    temp.desc = "CPU for general core " + number + " executed store instructions";
    list.push_back(temp);

    temp.name = prefix + ".general" + number + ".insts.store";
    temp.desc = "CPU for general core " + number + " executed load instructions";
    list.push_back(temp);

    temp.name = prefix + ".general" + number + ".insts.arithmetic";
    temp.desc =
        "CPU for general core " + number + " executed arithmetic instructions";
    list.push_back(temp);

    temp.name = prefix + ".general" + number + ".insts.fp";
    temp.desc =
        "CPU for general core " + number + " executed floating point instructions";
    list.push_back(temp);

    temp.name = prefix + ".general" + number + ".insts.others";
    temp.desc = "CPU for general core " + number + " executed other instructions";
    list.push_back(temp);
  }
}
