...
Driver::Driver(Engine &e, Host *ph, SimpleSSD::ConfigReader &conf, SimpleSSD::CPU::CPU *pCPU)
    : BIL::DriverInterface(e), pHost(ph), pCPU(pCPU), totalLogicalPages(0), logicalPageSize(0) {
  pHIL = new SimpleSSD::HIL::HIL(conf, pCPU);
}
...
void Driver::submitIO(BIL::BIO &bio) {
  ...
  auto *pFunc = new std::function<void(uint64_t, bool)>(bio.callback2);
  ...
  req.context = new SimpleSSD::HIL::CallBackContext((void *)bio.id);
  req.function = [pFunc](uint64_t , void *context) {
    SimpleSSD::HIL::CallBackContext *pContext = (SimpleSSD::HIL::CallBackContext *)context;
    pFunc->operator()((uint64_t) pContext->context, pContext->readValidPage);
    delete pFunc;
    delete pContext;
  };
