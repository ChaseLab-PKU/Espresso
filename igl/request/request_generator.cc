...
RequestGenerator::RequestGenerator(uint8_t id, Engine &e, Host *ph, BIL::BlockIOEntry &b,
                                   std::function<void()> &f, ConfigReader &c)
    : IOGenerator(e, ph, b, f),
      id(id),
      io_submitted(0),
      io_count(0),
      read_count(0),
  ...
  interleaved = c.readBoolean(CONFIG_REQ_GEN, REQUEST_INTERLEAVED);
  interleaveTime = c.readUint(CONFIG_REQ_GEN, REQUEST_INTERLEAVE_TIME) * 1000 * 1000 * 1000;
  startTime = c.readUint(CONFIG_REQ_GEN, REQUEST_INTERLEAVE_START_TIME) * 1000 * 1000 * 1000;
...
void RequestGenerator::begin() {
  ...
  if (interleaved) {
    engine.scheduleEvent(submitEvent, startTime);
  }
  else {
    _submitIO(initTime);
  }
...
void RequestGenerator::printStats(std::ostream &out) {
  ...
  out << "Tick: " << endTime << std::endl;
  out << "Time (ps): " << initTime << " - " << endTime << " (" << endTime - initTime
      << ")" << std::endl;
  out << "I/O (bytes): " << io_submitted << " ("
      << std::to_string((double)io_submitted / (endTime - initTime) *
                        1000000000000. / 1000. / 1000.)
      << " MB/s)" << std::endl;
  out << "I/O (counts): " << io_count << " (Read: " << read_count
      << ", Write: " << io_count - read_count << ")" << std::endl;
  out << "*** End of Request Generator Statistics ***\n" << std::endl;
...
void RequestGenerator::getProgress(float &val) {
  ...
  if (time_based) {                           // Read-only variable after init
    ...
    if (io_size == 0) val = 1.0;
    else val = (float)io_submitted / io_size;
  }
}
...
void RequestGenerator::_submitIO(uint64_t) {
  ...
  /*SimpleSSD::debugprint(SimpleSSD::LOG_COMMON, "Check queue depth %" PRIu64 "", io_depth);
  if(io_depth == iodepth){
    SimpleSSD::debugprint(SimpleSSD::LOG_COMMON, "The io queue is full");
  }*/
...
void RequestGenerator::rescheduleSubmit(uint64_t breakTime) {
  ...
  if (interleaved) {
    if ((tick - startTime) / interleaveTime % 2 == 0) {
      if (io_depth < iodepth) {
      ...
      if (io_depth < iodepth) {
        engine.scheduleEvent(submitEvent, tick + breakTime + interleaveTime);
      }
    }
  }
  else {
