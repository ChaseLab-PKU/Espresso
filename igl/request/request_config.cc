...
const char NAME_INTERLEAVED[] = "interleaved";
const char NAME_INTERLEAVE_TIME[] = "interleaveTime";
const char NAME_INTERLEAVE_START_TIME[] = "startTime";
...
bool RequestConfig::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_INTERLEAVED)) {
    interleaved = convertBoolean(value);
  }
  else if (MATCH_NAME(NAME_INTERLEAVE_TIME)) {
    interleaveTime = convertInteger(value);
  }
  else if (MATCH_NAME(NAME_INTERLEAVE_START_TIME)) {
    startTime = convertInteger(value);
  }
  else {
    ret = false;
  }
...
uint64_t RequestConfig::readUint(uint32_t idx) {
  ...
  switch (idx) {
    ...
    case REQUEST_RUN_TIME:
      ret = runtime;
      break;
    case REQUEST_INTERLEAVE_TIME:
      ret = interleaveTime;
      break;
    case REQUEST_INTERLEAVE_START_TIME:
      ret = startTime;
      break;
  }
