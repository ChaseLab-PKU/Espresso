...
const char NAME_BANDWIDTH_LOG_FILE[] = "BandwidthLogFile";
const char NAME_RETRY_LOG_FILE[] = "RetryLogFile";
...
const char NAME_OPENBLOCK_FILE[] = "OpenBlockFile";
const char NAME_OPCOUNT_FILE[] = "OpCountFile";
...
bool Config::setConfig(const char *name, const char *value) {
  ...
  else if (MATCH_NAME(NAME_OPENBLOCK_FILE)){
    openblockFile = value;
  }
  else if (MATCH_NAME(NAME_OPCOUNT_FILE)){
    opcountFile = value;
  }
  else if (MATCH_NAME(NAME_RETRY_LOG_FILE)){
    retrylogFile = value;
  }
  else if (MATCH_NAME(NAME_LATENCY_LOG_FILE)) {
    latencyFile = value;
  }
  else if (MATCH_NAME(NAME_BANDWIDTH_LOG_FILE)) {
    bandwidthFile = value;
  }
...
std::string Config::readString(uint32_t idx) {
  ...
  switch (idx) {
    ...
    case GLOBAL_LATENCY_LOG_FILE:
      ret = latencyFile;
      break;
    case GLOBAL_BANDWIDTH_LOG_FILE:
      ret = bandwidthFile;
      break;
    case GLOBAL_OPENBLOCK_FILE:
      ret = openblockFile;
      break;
    case GLOBAL_OPCOUNT_FILE:
      ret = opcountFile;
      break;
    case GLOBAL_RETRYLOG_FILE:
      ret = retrylogFile;
      break;
  }
