...
typedef struct _BIO {
  ...
  //temporary use for outputing whether read valid page in FTL
  std::function<void(uint64_t, bool)> callback2;
...
class BlockIOEntry {
 ...
 public:
  BlockIOEntry(ConfigReader &, Engine &, Host *, DriverInterface *, std::ostream *);
  ~BlockIOEntry();
  ...
  void getStat(uint64_t &, uint64_t &, uint64_t &);
};
