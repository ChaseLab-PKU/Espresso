...
BlockIOEntry::BlockIOEntry(ConfigReader &c, Engine &e, Host *ph, DriverInterface *i,
                           std::ostream *o)
    : conf(c),
      engine(e),
      pHost(ph),
      ...
      sumLatency(0),
      squareSumLatency(0),
      callback([this](uint64_t id) { completion(id, true); }),
      callback2([this](uint64_t id, bool readValidPage) { completion(id, readValidPage); }) {
  
  *pLatencyFile << "id,type,offset,length,cpl_time,latency,validRead" <<std::endl;
...
void BlockIOEntry::submitIO(BIO &bio) {
  ...
  copy.callback2 = callback2;

  pScheduler->submitIO(copy);
}

void BlockIOEntry::completion(uint64_t id, bool readValidPage) {
    ...
    if (iter->id == id) {
...

      {
        ...
        if (iter->type == BIO_READ) {
          readData += iter->length;
          ioData += iter->length;
        }
        else if (iter->type == BIO_WRITE) {
          writeData += iter->length;
          ioData += iter->length;
        }
      }
      if(readValidPage){
      ...
      if (pLatencyFile) {
        *pLatencyFile << std::to_string(iter->id) << ", "
                      ...
                      << std::to_string(iter->submittedAt + tick) << ", "
                      << std::to_string(tick) << ", "
                      << std::to_string(validRead) << std::endl;
      }
...
void BlockIOEntry::printStats(std::ostream &out) {
  ...
  out << "*** End of Block I/O Entry Statistics ***\n" << std::endl;
}
...
void BlockIOEntry::getStat(uint64_t &r, uint64_t &w, uint64_t &io) {
  r = readData;
  w = writeData;
  io = ioData;
}
